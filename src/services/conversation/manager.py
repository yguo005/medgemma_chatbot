...
class ConversationManager:
    ...
    def _construct_final_prompt(self, session: Dict[str, Any]) -> str:
        ...
        Your task is to provide a detailed medical analysis based on all available symptoms.
        Consider possible conditions and explain your reasoning.

        IMPORTANT: Your response MUST NOT include any section for "Recommended Next Steps" or "Recommendations".
        This information is generated by a separate system.
        Focus ONLY on the medical analysis of possible conditions.
        """
        return final_prompt
...
{{
  ...
  "severity_indicators": ["severe"]
        ...
        try:
            # OPTIMIZATION: Always use OpenAI for symptom extraction (fast and cost-effective)
            # Reserve MedGemma Model Garden for the final diagnostic summary
            if not (hasattr(self.ai_service, 'services') and self.ai_service.services.get('openai')):
            ...
            logger.info(f"✅ OpenAI symptom extraction successful: {structured_data}")
            return structured_data
            
        except Exception as e:
            logger.error(f"❌ OpenAI symptom extraction failed: {e}")
